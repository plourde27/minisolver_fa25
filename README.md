...
5. In minisolver2.py, update the list of problems on line 9 to be the ordered list of problem names for the current contest
6. In a terminal window, cd into the minisolver directory and run "python minisolver2.py"
7. On lines 122 and 123 of index.html, update window.problemScores and window.nSubparts with the scores and number of subparts for each problem of the current contest, respectively
8. On lines 505-507 of index.html, update all of "window.gold", "window.silver", and "window.bronze" to equal the number of in-person teams
9. Run the frontend locally, i.e. run "python -m http.server 8000" and open "0.0.0.0:8000/index.html" and follow the instructions onscreen to play the resolver during the ceremony
